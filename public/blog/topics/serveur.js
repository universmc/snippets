...
app.get('/IA_et_cinema', async (req, res) => {
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                { role: 'assistant', content: `** **<br/> | in box |.`, },
                {
                    role: 'user',
                    content: `** Rédige un article de blog sur l'IA et ses applications dans le domaine du Cinema. Ta réponse doit être rédigé au format liste en HTML, respectant les normes du Web sémantique W3C intégrant des emoji intélligent associer`,
                },
            ],
            model: 'gemma2-9b-it',
        });
...
app.get('/IA_et_jeux_video', async (req, res) => {
    try {
        const chatCompletion = await groq.chat.completions.create({
            messages: [
                { role: 'assistant', content: `** **<br/> | in box |.`, },
                {
                    role: 'user',
                    content: `** Rédige un article de blog sur l'IA et ses applications dans le domaine de l'IA et les Jeux Video. Ta réponse doit être rédigé au format liste en HTML, respectant les normes du Web sémantique W3C intégrant des emoji intélligent associer`,
                },
            ],
            model: 'gemma2-9b-it',
        });
